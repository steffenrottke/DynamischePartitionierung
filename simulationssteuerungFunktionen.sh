...
ungueltigesRealisierungskonzept () {
	if ! { [ "$konzept" = "f" ] || [ "$konzept" = "b" ] || [ "$konzept" = "n" ] || [ "$konzept" = "w" ] || [ "$konzept" = "r" ]; }; then
		echo "Die Eingabe enthaelt nicht eins der fuenf aufgelisteten Realisierungskonzepte. Bitte waehlen Sie erneut ein Realisierungskonzept aus."
	fi
}
